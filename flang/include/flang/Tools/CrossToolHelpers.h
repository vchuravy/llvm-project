...
public:
  ...
  // The HLFIR extension points run inside the HLFIR-to-FIR pass pipeline,
  // while HLFIR intrinsic operations (hlfir.sum, hlfir.matmul, ...) are still
  // present. "Early" runs before any HLFIR simplification/inlining; "Last"
  // runs right before the intrinsics are lowered to FIR/runtime calls. These
  // are const so they can be invoked on a const pipeline config.
